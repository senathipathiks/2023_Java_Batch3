...
<%
	...
	if(action.equals("Insert")){
		...
		<%="<tr><td>"+emp.getEmpId()+"</td><td>"+emp.getEmpName()+"</td><td>"+emp.getEmpAge()+"</td></tr></table></div>" %>
		<%	
		...
		%>
		<%="<br><div class=\"card card-body w-50\"><h4>No Of Record : "+list.size()+"</h4>" %>
		<%="<br><table><tr style=\"font-weight:bold\"><td>Employee ID</td><td>Employee Name</td><td>Employee Age</td><tr>" %>
		<%
		for(Employee emp : list){
			%>
			<%="<tr><td>"+emp.getEmpId()+"</td><td>"+emp.getEmpName()+"</td><td>"+emp.getEmpAge()+"</td></tr>" %>
			<%} %>
			<%="</table></div>" %>
			<%
		}
