...
import com.btbs.dao.BusDAO;

@WebServlet("/")
public class Controller extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static BusDAO dao = new BusDAO();
	...
	private void viewAll(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		LinkedList<Bus> list = dao.veiwAllBooking();
		if(list != null) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action.equals("Insert")) {
				Bus b = new Bus(Integer.parseInt(request.getParameter("ticketId")), request.getParameter("boarding"), request.getParameter("destination"), Float.parseFloat(request.getParameter("price")), Integer.parseInt(request.getParameter("noOfSeat")));
				n= dao.insertBooking(b);
				if(n==1) {
			...
			boolean flag = dao.deleteAllBooking(pass);
			if(flag) {
